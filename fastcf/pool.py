...
每个 DC（colo）维护最多 POOL_SIZE 个已验证可直连的 IP。
任何探测行为只要读到实际服务节点（cf-meta-colo），就把 IP 写回
**实际命中的那个 DC**（无目标过滤）。

入池途径（无后台线程，全部前台/事件性）：
  1. 手动添加（probe_and_add：已知来源校验 + cf-meta-colo 归池）
  2. 扫描中随机 IP 测速前探测入池
  3. 测速成功的 IP 回写其实际 DC

TTL 语义：池按"最后入池时间"过期（7 天）。过期**不删除** IP、不后台重探；
当指定 DC 扫描用到该池时触发**事件性**重新探测（scanner 前台同步执行）。
...
from .speedtest import probe_location
...
def probe_and_add(ips: list, code_hint: str = "", workers: int = 12, log=None) -> dict:
    ...
    cidrs = sources.fetch_cf_ips().get("v4", [])
    try:
        ext_ips = sources.fetch_external_ips().get("v4", [])
    except Exception:
        ext_ips = []
    ...
    for raw in ips:
        ...
        if not ip:
            continue
        if not sources.is_known_ip(ip, cidrs, ext_ips):
    ...
    def probe(ip):
        _cc, colo, _city = probe_location(ip)
        if not colo:
