...
**诊断结论**：本环境到 CF 的 TCP/TLS 握手很快（0.2s），但**数据流被限流到 ~1.5Mbps**（3 秒只收到 255KB，正好是请求的 256KB 总量）。
- 直连诊断 5 个已知 CF IP：3 个可达（~1.4-1.6Mbps），2 个 TCP 超时
- 历史数据（8 月底）本环境曾跑到 374Mbps → 确认是**网络出口限速**，不是代码 bug
- 真实扫描（RANDOM 15 IP / 10 IP）：全部 0Mbps → "没有可测速的 Cloudflare IP"（预期行为）
- 单元测试 `test_appstate`（真实网络）：2 次扫描各返回 2 个达标结果（3-9Mbps），功能正常
- 结论：代码逻辑正确；本环境当前网络条件不适合测速（CF 数据流被限速），生产环境网络正常时速度会快很多
