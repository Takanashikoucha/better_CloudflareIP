...
C. 下载测速（443/TLS，4 路并发，按延迟升序提交）
   ...
   限速保护：单 IP 流量预算 min(设定, 20MB)；相邻 IP 测速间隙 0.5s；
   429 双层退避（Retry-After 短冷却 ≤30s + 全局冷静 10s 起步/指数递增/≤60s，成功 IP 递减复位）
...
- 简化原则：默认只露 2 层控件（模式 + 快速/精准预设）；高级参数（时长/流量/下限/随机数）折叠；
  历史/IP 池/系统信息收进右侧抽屉；结果表 7 列 → 5 列（#/IP/节点/延迟/速度）
- 交互：SSE 增量日志流（logDelta）、进度条、结果表 rank 徽章 + 按列排序、历史复用参数、
  抽屉（Esc 关闭）、429 限速提示条、GFW 提示条、toast 反馈
- 动效克制：状态点 pulse、进度条填充、抽屉滑入；`prefers-reduced-motion` 支持
