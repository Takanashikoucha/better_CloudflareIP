...
- 🗄️ **DC 级 IP 池 + 后台常驻填充** — 每个 DC 缓存已验证 IP（50/节点）；启动即起后台填充线程，持续为就近 DC（默认 US/JP/HK 组）采样建池。**任何探测行为只要读到实际服务节点（`cf-meta-colo`），IP 就写回实际命中的 DC**
