...
- 🛡️ **Cloudflare 限速保护** — 多层防御避免触发 429：单 IP 流量预算（≤20MB）· 相邻 IP 测速间隙 0.5s ·
  429 时双层退避（按 Retry-After 短冷却 ≤30s + 全局冷静期 10s 起步、指数递增、≤60s）· 限速状态实时透出到 UI
...
用户只需 2 步：**选来源（指定 DC 或 全局随机）· 选测速预设（快速/精准）**，点击开始。
高级参数（测速时长 / 流量 / 速度下限 / 随机 IP 数量）在「高级选项」里展开。
...
- `speedSecs`：每个候选 IP 的测速时长（3–60s，默认 5）
- `speedMB`：下载流量上限（1–20MB，默认 5；服务端另设 20MB 单 IP 流量预算，超限自动截断）
